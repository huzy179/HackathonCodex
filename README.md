...
* **Tối ưu hóa Chi phí & Độ trễ:** Hệ thống tự động bỏ qua Agent 2 nếu độ tự tin bóc tách của Agent 1 cao (>= 85%) cho tất cả các trường, giúp tăng tốc độ phản hồi lên đến 50%.
...
Mọi hành động (Upload file, AI bóc tách đa tác nhân, sửa đổi tay thủ công, ký duyệt) đều được hệ thống ghi nhận thời gian thực và lưu trữ lâu dài vào cơ sở dữ liệu SQLite ở Backend, đồng bộ tự động với Frontend (không bị mất dữ liệu khi tải lại hoặc F5 trang).
...
### 8. Hỗ trợ PDF Nhiều Trang (Multi-page PDF Candidate Scan)
Hệ thống tự động quét nhanh văn bản thô của tối đa 10 trang đầu của tài liệu PDF tải lên, tính toán điểm dựa trên mức độ tập trung của các từ khóa hóa đơn thương mại (như invoice, total, beneficiary...). Trang có điểm số cao nhất chứa nhiều dữ liệu hóa đơn nhất sẽ được tự động chọn để render thành ảnh JPEG base64 gửi cho OpenAI Vision.
